...
class CaptchaService
{
    ...
    {
        if ($this->config->has('modules.captcha.' . $config)) {
            foreach ($this->config->get('modules.captcha.' . $config) as $key => $val) {
                $this->{$key} = $val;
            }
        }
    }
    ...
    {
        $this->backgrounds = \Illuminate\Support\Facades\File::files(module_path('Captcha', 'resources/assets/backgrounds'));
        $this->fonts = \Illuminate\Support\Facades\File::files($this->fontsDirectory);
